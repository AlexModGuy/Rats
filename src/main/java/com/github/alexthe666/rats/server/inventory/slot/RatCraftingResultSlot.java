...
public class RatCraftingResultSlot extends SlotItemHandler {
	...
	protected void checkTakeAchievements(ItemStack stack) {
		...
		Optional<RecipeHolder<CraftingRecipe>> recipeHolder = this.table.getGuideRecipe();
		recipeHolder.ifPresent(holder -> {
			if (!holder.value().isSpecial()) {
				this.player.awardRecipes(Lists.newArrayList(holder));
			}
		});
	}
