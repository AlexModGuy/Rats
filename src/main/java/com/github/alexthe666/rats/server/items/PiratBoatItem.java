...
public class PiratBoatItem extends Item {

	private static final Predicate<Entity> ENTITY_PREDICATE = EntitySelector.NO_SPECTATORS.and(Entity::isPickable);
	private final PiratWoodBoat.Type type;
	private final boolean chest;

	public PiratBoatItem(boolean chest, PiratWoodBoat.Type type, Properties properties) {
		super(properties);
		this.chest = chest;
		this.type = type;
	}
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (result.getType() == HitResult.Type.MISS) {
			...
			if (result.getType() == HitResult.Type.BLOCK) {
				PiratWoodBoat boat = this.getBoat(level, result);
	...
	private PiratWoodBoat getBoat(Level level, HitResult result) {
