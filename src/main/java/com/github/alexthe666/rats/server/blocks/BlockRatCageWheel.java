...
public class BlockRatCageWheel extends BlockRatCage implements ITileEntityProvider, INoTab {
    ...
    public BlockRatCageWheel() {
        super("rat_cage_wheel", Properties.create(Material.IRON).sound(SoundType.METAL).notSolid().variableOpacity().hardnessAndResistance(2.0F, 0.0F));
    ...
    public void updateTick(World worldIn, BlockPos pos, BlockState state, Random rand) {
        ...
        if (worldIn.getTileEntity(pos) != null && worldIn.getTileEntity(pos) instanceof TileEntityRatCageWheel) {
            TileEntityRatCageWheel te = (TileEntityRatCageWheel) worldIn.getTileEntity(pos);
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (worldIn.getTileEntity(pos) != null && worldIn.getTileEntity(pos) instanceof TileEntityRatCageWheel && state.getBlock() != newState.getBlock()) {
            TileEntityRatCageWheel te = (TileEntityRatCageWheel) worldIn.getTileEntity(pos);
