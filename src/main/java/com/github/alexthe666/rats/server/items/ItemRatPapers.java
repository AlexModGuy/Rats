...
public class ItemRatPapers extends Item {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if(!isEntityBound(stack)){
            tooltip.add(new TranslationTextComponent("item.rats.rat_papers.desc0").applyTextStyle(TextFormatting.GRAY));
            tooltip.add(new TranslationTextComponent("item.rats.rat_papers.desc1").applyTextStyle(TextFormatting.GRAY));
        }
        tooltip.add(new TranslationTextComponent("item.rats.rat_papers.desc2").applyTextStyle(TextFormatting.GRAY));
        if (stack.getTag() != null) {
            tooltip.add(new TranslationTextComponent("item.rats.rat_papers.rat_desc").applyTextStyle(TextFormatting.GRAY));
            ...
            if (stack.getTag().hasUniqueId("RatUUID")) {
                if(entity.isEmpty()){
                    tooltip.add(new TranslationTextComponent(ratName).applyTextStyle(TextFormatting.GRAY));
                }else{
                    tooltip.add(new StringTextComponent(entity).applyTextStyle(TextFormatting.GRAY));
                }
            }
        }
    }
