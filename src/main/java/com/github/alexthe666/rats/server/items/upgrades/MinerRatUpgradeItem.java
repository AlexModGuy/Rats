...
public class MinerRatUpgradeItem extends RatListUpgradeItem implements HoldsItemUpgrade, ChangesAIUpgrade, DamageImmunityUpgrade {
	...
	public void renderHeldItem(EntityRendererProvider.Context context, TamedRat rat, RatModel<?> model, PoseStack stack, MultiBufferSource buffer, int light, float ageInTicks) {
		...
		context.getItemRenderer().renderStatic(new ItemStack(Items.DIAMOND_PICKAXE), ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, light, OverlayTexture.NO_OVERLAY, stack, buffer, null, rat.getId());
		stack.popPose();
	}
