...
public class RatSackItem extends Item {
	...
	public static void packRatIntoSack(ItemStack sack, TamedRat rat, int ratCount, HolderLookup.Provider registries) {
		CompoundTag tag = sack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
		CompoundTag ratTag = new CompoundTag();
		rat.addAdditionalSaveData(ratTag);
		if (rat.hasCustomName()) {
			ratTag.putString("CustomName", Component.Serializer.toJson(rat.getCustomName(), registries));
		}
	...
	public static int getRatsInSack(ItemStack sack) {
		int ratCount = 0;
		CustomData customData = sack.get(DataComponents.CUSTOM_DATA);
		if (customData != null) {
			...
			for (String tagInfo : tag.getAllKeys()) {
				if (tagInfo.contains("Rat")) {
					...
					if (!ratTag.getString("CustomName").isEmpty()) {
						rat.setCustomName(Component.Serializer.fromJson(ratTag.getString("CustomName"), level.registryAccess()));
					}
		...
		if (customData != null) {
			...
			for (String tagInfo : tag.getAllKeys()) {
				if (tagInfo.contains("Rat")) {
					...
					if (!ratTag.getString("CustomName").isEmpty()) {
						// Note: In tooltips we don't have a proper registry access, use literal parsing
						try {
							Component ratNameTag = Component.literal(ratTag.getString("CustomName"));
							if (ratNameTag != null) {
