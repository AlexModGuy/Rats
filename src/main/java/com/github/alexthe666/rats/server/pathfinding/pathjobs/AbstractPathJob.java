...
public abstract class AbstractPathJob implements Callable<Path> {
    ...
    public static Direction getAnyFacing(final BlockPos pos, final BlockPos neighbor) {
        final BlockPos vector = neighbor.subtract(pos);
    ...
    protected Path search() {
        ...
        while (!nodesOpen.isEmpty()) {
            ...
            if (totalNodesVisited > RatConfig.maxRatPathingNodes || totalNodesVisited > maxRange * maxRange) {
                break;
            }
    ...
    private void handleDebugOptions(final Node currentNode) {
        ...
        if (Pathfinding.isDebug()) {
            RatsMod.LOGGER.info(String.format("Examining node [%d,%d,%d] ; g=%f ; f=%f",
    ...
    private Node getAndSetupStartNode() {
        ...
        startNode.setOnRails(pathingOptions.canUseRails());
    ...
    private void doDebugPrinting(final PathPoint[] points) {
        if (Pathfinding.isDebug()) {
            ...
            for (final PathPoint p : points) {
                RatsMod.LOGGER.info(String.format("Step: [%d,%d,%d]", p.x, p.y, p.z));
            }

            RatsMod.LOGGER.info(String.format("Total Nodes Visited %d / %d", totalNodesVisited, totalNodesAdded));
        }
    }
    ...
    protected final boolean walk(final Node parent, BlockPos dPos) {
        ...
        final boolean onRails = pathingOptions.canUseRails();
    ...
    protected boolean isPassableBB(final BlockPos parentPos, final BlockPos pos) {
        if (circumventSizeCheck) {
            return isPassableRattube(pos, false, parentPos) ;
        } else {
