...
public class ChristmasRatUpgradeItem extends BaseRatUpgradeItem implements TickRatUpgrade, HoldsItemUpgrade {
	...
	private void tryGiftgiving(TamedRat rat) {
		...
		if (!rat.level().isClientSide()) {
			...
			List<ItemStack> result = rat.level().getServer().reloadableRegistries().getLootTable(RatsLootRegistry.CHRISTMAS_GIFTS_KEY).getRandomItems(builder.create(lootparameterset$builder.build()));
			if (RatsDateFetcher.isChristmasDay()) {
				for (int i = 0; i < 5; i++) {
					result.addAll(rat.level().getServer().reloadableRegistries().getLootTable(RatsLootRegistry.CHRISTMAS_GIFTS_KEY).getRandomItems(builder.create(lootparameterset$builder.build())));
				}
			}
			if (!result.isEmpty()) {
	...
	public void renderHeldItem(EntityRendererProvider.Context context, TamedRat rat, RatModel<?> model, PoseStack stack, MultiBufferSource buffer, int light, float ageInTicks) {
		...
		VertexConsumer consumer = buffer.getBuffer(RenderType.entityCutoutNoCull(ResourceLocation.withDefaultNamespace("textures/entity/chest/christmas.png")));
		RatHeldItemLayer.CHRISTMAS_CHEST_MODEL.renderToBuffer(stack, consumer, light, OverlayTexture.NO_OVERLAY, -1);
		stack.popPose();
		stack.popPose();
	}
}
