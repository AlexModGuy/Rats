...
public class ModelRat<T extends EntityRat> extends AdvancedEntityModel<T>{
    ...
    public void setRotationAngles(T rat, float f, float f1, float f2, float f3, float f4) {
        ...
        if(tailBehavior == 4){
            progressRotation(tail1, rat.sitProgress, 0.5F, 0, 0, 20F);
            progressRotation(tail2, rat.sitProgress, -0.3F, 0, 0, 20F);
        }
        if(tailBehavior == 5){
            progressRotation(tail2, rat.sitProgress, 0.3F, 0.6108652381980153F, 0.0F, 20F);
            progressRotation(tail1, rat.sitProgress, 1.1F, 0.17453292519943295F, 0.6981317007977318F, 20F);
        }
        ...
        if ((rat.hasUpgrade(RatsItemRegistry.RAT_UPGRADE_CRAFTING) || rat.hasUpgrade(RatsItemRegistry.RAT_UPGRADE_LUMBERJACK) || rat.hasUpgrade(RatsItemRegistry.RAT_UPGRADE_QUARRY)  || rat.hasUpgrade(RatsItemRegistry.RAT_UPGRADE_MINER) || rat.hasUpgrade(RatsItemRegistry.RAT_UPGRADE_MINER_ORE) || rat.hasUpgrade(RatsItemRegistry.RAT_UPGRADE_FISHERMAN)) && rat.crafting) {
