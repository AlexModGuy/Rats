...
public class PlagueLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
	...
	public void render(PoseStack stack, MultiBufferSource buffer, int light, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		// In 1.21, hasEffect expects Holder<MobEffect>, DeferredHolder implements Holder so pass directly
		if (!(entity instanceof AbstractRat) && entity.hasEffect(RatsEffectRegistry.PLAGUE)) {
			this.getParentModel().renderToBuffer(stack, buffer.getBuffer(TEXTURE), light, OverlayTexture.NO_OVERLAY, -1);
		}
	}
}
